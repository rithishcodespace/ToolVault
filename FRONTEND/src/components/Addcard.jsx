...
let Addcard = () =>{
    ...
    async function handleAddCard()
    {
        ...
        {
          const response = await axios.post(`http://localhost:1234/addcard/${table}`,{
            img:url.current.value,
            name:name.current.value,
            details:highlights.current.value,
            rate:rate.current.value,
            extradetails:description.current.value,
    ...
    return(
        <div className="flex justify-center">
            ...
            <p className="text-mono text-2xl relative bottom-3.5">Add Card</p>
            ...
            <button className="h-12 w-[500px] cursor-pointer rounded-lg bg-blue-950 relative top-3 text-white hover:bg-blue-900" onClick={handleAddCard}>Click to Add the Card</button>
            </div>
        </div>
            )
}
